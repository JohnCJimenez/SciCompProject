...
disp('Solves the 2D Helmholtz Equation within a square domain using Gauss-Seidel method')
...
%% Boundary Conditions Setup and Evaluation of Relevant Functions
    ...
    % boundary conditions as well as the function F(x,y)   
        ...
        % F = zeros(N,N); % This is for the Final Simulation only
...
%% The Gauss-Seidel Method Execution
   % Node Evaluation
    while e > Es
    U1 = U;
    for a = 2:IntNodesX-1
    ...
    e = max(max(abs((U-U1)./U))); % Relative Error Calculation
end
