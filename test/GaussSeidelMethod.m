...
% through the iterative Gauss-Seidel Method
...
%% Dimensions of the Square Domain and Some Important Parameters
    ...
    % A value for Lambda can be assigned directly but is left up to user input for greater versatility 
        ...
        Lambda_final = input('Please input a value for Lambda for the final simulation:'); % Specific value of Lambda for final simulation.
...
%% Assembling the Coefficient Matrix & the F(x,y) Vector so that it takes the form of Au = F (A is a pentadiagonal, square matrix)
    ...
    % Creating the F(x,y) vector: 
    
        F       = Fxy(xval,yval)*B;     % Normalized with constant multiplier B. See report for details.
        F_final = zeros(size(F));       % F(x,y) for Final simulation
